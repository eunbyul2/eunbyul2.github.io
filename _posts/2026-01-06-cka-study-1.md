...
### 1-3) apply 경고: last-applied-configuration
...
### 5-3) metadata 아래 name은 항상 있어야 함?
...
### 12-1) kubectl get svc에서 보이는 포트는 무엇인가?
- PORT(S) 컬럼은 Service의 spec.ports[].port만 보여줌.
- targetPort는 요약 출력에 안 나옴.

### 12-2) “kubernetes service의 targetPort는 무엇인가?” 문제 풀이법
...
### 12-3) “kubernetes service에 labels가 몇 개인가?” 문제 풀이법
...
### 16-2) Service 접속이 안 될 때
- selector와 Pod labels 일치 확인
- endpoints가 비어 있는지 확인

### 16-3) Pod가 Running이 안 될 때
